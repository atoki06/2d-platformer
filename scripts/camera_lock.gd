...
func change_position_type() -> void:
	save_point_positions()
	delete_all_points()
	create_points(pow(2,position_type))
	
func add_missing_nodes() -> void:
...
func save_point_positions() -> void:
	point_positions = []
	for point : Node in position_limit.get_children():
